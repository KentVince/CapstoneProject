...
>
    {{-- ── Top Section: Image + Details Grid ─────────────────────────────── --}}
            ...
            @if($info)
                <div class="col-span-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 p-3 rounded-lg">
                    ...
                    <p class="mt-1 text-xs text-blue-600 dark:text-blue-400">
                        <span class="font-medium">Severity Scale:</span> {{ $info['severity_levels'] }}
                    </p>
                </div>
            @endif
        </div>
    ...
    <div class="flex items-center justify-between bg-gray-700 dark:bg-gray-900 rounded-lg px-4 py-2 shadow-sm">
        <div class="flex items-center gap-2 text-white text-xs font-medium">
            ...
            <span>Pest &amp; Disease Report &mdash; {{ $record->pest }}</span>
        </div>
        <button
            type="button"
        ...
        <div class="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
            ...
            <div class="p-4 bg-white dark:bg-gray-900 min-h-[180px]">
                        ...
                        @foreach($info['seven_day_plan'] as $step)
                            <div class="flex gap-3 items-start">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-600 dark:text-gray-400 mt-0.5">{{ $step['detail'] }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div x-show="activeTab === 'Immediate Response'" style="display:none;">
                    ...
                    <ul class="space-y-2">
                        @foreach($info['immediate_response'] as $idx => $item)
                            <li class="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300">
                ...
                <div x-show="activeTab === 'Long-term Strategy'" style="display:none;">
                    <ul class="space-y-2">
                        @foreach($info['long_term'] as $item)
                            <li class="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300">
    ...
    @else
        {{-- Fallback if pest name not in static database --}}
        <div class="border border-gray-200 dark:border-gray-700 rounded-xl p-4 bg-gray-50 dark:bg-gray-800">
            <p class="text-sm text-gray-500 dark:text-gray-400 text-center">
                No reference data available for "<strong>{{ $pestName }}</strong>". Please consult the expert recommendation below.
            </p>
        </div>
    @endif
